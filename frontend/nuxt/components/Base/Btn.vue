...
const { icon, to, label, color, hasChip, isMenu, mobile, isPopover } =
  props.btnOptions;

const isActiveInSameRoute = computed(() => TRoute.path === to)

</script>

<template>
  <UButton v-bind:[attr({isMenu,isPopover})]="to" size="lg" square color="gray"
    ...
    <span class="absolute left-0 top-0 block h-full w-full rounded-none md:hidden md:rounded-full"
      :class="{ '!bg-blue-600 !text-white': TRoute.path === to }" />
